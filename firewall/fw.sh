...
SYSCTL="/sbin/sysctl"
BLOCKEDIPS="/root/firewall/blockedips.txt"
#ALLOW_SSH="/root/firewall/ssh.allowed"
...
# Block sync
$IPT -A INPUT -i ${PUB_IF} -p tcp ! --syn -m state --state NEW  -m limit --limit 5/m --limit-burst 7
#-j LOG --log-level 4 --log-prefix "Drop Sync"
...
#SSHALLOW=$(grep -Ev "^#" $ALLOW_SSH)
#for i in $SSHALLOW
#do
#   $IPT -A INPUT -i ${PUB_IF} -p tcp --destination-port 22 -s $i -j ACCEPT
#done
...
$IPT -A INPUT -i ${PUB_IF} -p tcp --destination-port 9090 -j ACCEPT
